...
if __name__ == '__main__':
    ...
    for model_name in ['svm']:
        for data in dataset:
            ...
            plot_compare_auc(dataset_res,'dataset',var_name(data)[0],'{}/meta_and_composition'.format(model_name))
            dataset_res = pd.DataFrame(index=[], columns=['x','y','datasetname'] )

            plot_compare_auc(disease_res,'disease',var_name(data)[0],'{}/meta_and_composition'.format(model_name))
            disease_res = pd.DataFrame(index=[], columns=['x','y','datasetname'] )